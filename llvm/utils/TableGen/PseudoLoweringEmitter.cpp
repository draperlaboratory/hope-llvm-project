  ...
  if (!Expansions.empty()) {
    ...
    for (auto &Expansion : Expansions) {
      ...
      o << "    case " << Source.Namespace << "::"
        ...
        << "      MCOperand MCOp;\n"
	<< "\n"
	<< "      if(MI->getFlag(MachineInstr::FnProlog)) {TmpInst.setISPMetadataTag(DMT_STACK_PROLOGUE_AUTHORITY); }\n"
	<< "      else if(MI->getFlag(MachineInstr::FnEpilog)){ TmpInst.setISPMetadataTag(DMT_STACK_EPILOGUE_AUTHORITY); }\n"
	<< "      else if(MI->getFlag(MachineInstr::FPtrStore)){ TmpInst.setISPMetadataTag(DMT_FPTR_STORE_AUTHORITY); }\n"
	<< "      else if(MI->getFlag(MachineInstr::FPtrCreate)){ TmpInst.setISPMetadataTag(DMT_FPTR_CREATE_AUTHORITY); }\n"
	<< "\n"
	<< "      if ( MI->getFlag(MachineInstr::CallTarget) ) { TmpInst.setISPMetadataTag(DMT_CFI3L_VALID_TGT); }\n"
	<< "      if ( MI->getFlag(MachineInstr::ReturnTarget) ) { TmpInst.setISPMetadataTag(DMT_RET_VALID_TGT); }\n"
	<< "      if ( MI->getFlag(MachineInstr::BranchTarget) ) { TmpInst.setISPMetadataTag(DMT_BRANCH_VALID_TGT); }\n"
	<< "\n"
	<< "      if(MI->isReturn() && !MI->isCall()){ TmpInst.setISPMetadataTag(DMT_STACK_EPILOGUE_AUTHORITY); TmpInst.setISPMetadataTag(DMT_RETURN_INSTR); } \n"
	<< "      else if(MI->isCall()) { TmpInst.setISPMetadataTag(DMT_CALL_INSTR); }\n"
	<< "      else if(MI->isBranch()) { TmpInst.setISPMetadataTag(DMT_BRANCH_INSTR); }\n"
	<< "\n"
